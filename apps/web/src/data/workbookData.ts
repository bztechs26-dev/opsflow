...
function loadStatus(sourceStatus = ''): Load['status'] { const status = sourceStatus.toLowerCase(); if (/deliver|in[ _-]?transit/.test(status)) return 'DISPATCHED'; if (/loaded/.test(status)) return 'LOADED'; if (/delay/.test(status)) return 'DELAYED'; if (/issue|declin/.test(status)) return 'ISSUE'; if (/cancel|close/.test(status)) return 'CLOSED'; if (/stage/.test(status)) return 'STAGED'; return 'NOT_STARTED' }
