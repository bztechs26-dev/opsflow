...
export function CapacityStaffingPlanner({ records, plan, onChange }: Props) {
  ...
  const [lhptText, setLhptText] = useState<string | null>(null)
  ...
  return <section className="capacity-staffing-page">
    <section className="panel capacity-planner">
      ...
      <div className="capacity-inputs">
        <label>Machine<select value={selectedMachine} onChange={(event) => { setMachine(event.target.value); setLhptText(null) }}>{machines.map((name) => <option key={name} value={name}>{name}</option>)}</select><small>Select the machine being planned.</small></label>
        ...
        <label>LHPT goal<input inputMode="decimal" value={lhptText ?? (selected.lhptGoal || '')} placeholder="e.g. 0.075" onChange={(event) => { const value = event.target.value; if (!/^\d*(?:\.\d*)?$/.test(value)) return; setLhptText(value); if (value && !value.endsWith('.')) updateMachine('lhptGoal', Number(value)) }} onBlur={() => { if (lhptText !== null) { updateMachine('lhptGoal', Number(lhptText || 0)); setLhptText(null) } }}/><small>Labor hours required per 1,000 pieces.</small></label>
