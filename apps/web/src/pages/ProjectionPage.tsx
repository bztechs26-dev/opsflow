...
export function ProjectionPage({ weeks, selectedWeekId, token = '', onDataChanged }: { weeks: OperationalWeek[]; selectedWeekId: string; token?: string; onDataChanged?: () => Promise<void> }) {
  ...
  const visible = projections.filter((item) => (selectedMarket === 'ALL' || item.market === selectedMarket) && (!tripSearch.trim() || normalizeTrip(item.trip).includes(normalizeTrip(tripSearch))))
  const upload = async () => {
