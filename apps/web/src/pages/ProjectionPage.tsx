...
export function ProjectionPage({ weeks, selectedWeekId, token = '', onDataChanged }: { weeks: OperationalWeek[]; selectedWeekId: string; token?: string; onDataChanged?: () => Promise<void> }) {
  ...
  const selectWeek = (value: string) => { setWeekId(value); setSelectedMarket('ALL'); setShippingStatus('ALL'); setDestination('ALL'); setCarrier('ALL'); setExpanded(new Set()) }
  ...
  return <section className="page projection-page">
    ...
    <section className="panel projection-upload"><div><span className="metric-label">Projection intake</span><h2>Upload a ZIP / TR workbook</h2><p>The filename identifies the market and Projection week: FE Wk 37, BE Wk 37, or Bost CT Hart Wk 37. ZIP is read from column A and TR from column C. Use .xlsx, .xlsm, .xls, or .csv.</p></div><div className="projection-form"><label>Projection week<select className="week-select" value={weekId} onChange={(event) => selectWeek(event.target.value)} disabled={!weeks.length}>{weeks.length ? weeks.map((item) => <option key={item.id} value={item.id}>{item.label}</option>) : <option value="">No uploaded weeks</option>}</select></label><label className="projection-file">ZIP / TR workbook<input ref={input} type="file" accept=".xlsx,.xlsm,.xls,.csv" onChange={() => setMessage('')} /></label><button className="primary-button" type="button" onClick={() => void upload()} disabled={isUploading}>{isUploading ? 'Importing...' : 'Import projection workbook'}</button></div>{message && <p className="projection-message">{message}</p>}</section>
