...
import { machineRate } from '../data/machineCapacity'
import type { Load, LoadStatus, OperationalWeek, ProductionRecord, ProductionStatus } from '../types/operations'

type ZipProgress = { zip: string; status: ProductionStatus | 'MISSING'; machine?: string; quantity?: number; remainingHours?: number }
type TripProjection = { market: string; trip: string; zips: ZipProgress[]; complete: number; percent: number; readiness: string; remainingHours?: number; carrier?: string; destination?: string; shippingStatus: LoadStatus | 'NOT_IN_BULK_PLAN' }
const labels: Record<string, string> = { FE: 'Front End', BE: 'Back End', 'PROV-BOST': 'Boston / CT / Hartford', MMSI: 'MMSI' }

export function ProjectionPage({ weeks, availableWeekIds = weeks.map((week) => week.id), selectedWeekId, token = '', onDataChanged, onWeekSelected, onEnsureWeeksLoaded }: { weeks: OperationalWeek[]; availableWeekIds?: string[]; selectedWeekId: string; token?: string; onDataChanged?: () => Promise<void>; onWeekSelected?: (week: string) => Promise<void>; onEnsureWeeksLoaded?: (weeks: string[]) => Promise<void> }) {
  ...
  useEffect(() => { if (selectedWeekId && selectedWeekId !== weekId) setWeekId(selectedWeekId) }, [selectedWeekId, weekId])
  const refreshMarkets = useCallback(async (requestedWeek = weekId) => {
    if (!requestedWeek) { setMarkets([]); return }
    try {
      const nextMarkets = await fetchProjectionMarkets(requestedWeek, token)
      setMarkets(nextMarkets)
      await onEnsureWeeksLoaded?.(nextMarkets.map((market) => String(market.productionSourceWeek)))
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Projection storage is unavailable.') }
  }, [onEnsureWeeksLoaded, token, weekId])
  useEffect(() => { void refreshMarkets() }, [refreshMarkets])
  const shippingLoads = useMemo(() => weeks.find((week) => week.id === weekId)?.loads ?? [], [weeks, weekId])
  const projections = useMemo(() => deriveTrips(markets, weeks, shippingLoads), [markets, weeks, shippingLoads])
  const carriers = useMemo(() => [...new Set(projections.map((item) => item.carrier).filter((value): value is string => Boolean(value)))].sort(), [projections])
  const destinations = useMemo(() => [...new Set(projections.map((item) => item.destination).filter((value): value is string => Boolean(value)))].sort(), [projections])
  const shippingStatuses = useMemo(() => [...new Set(projections.map((item) => item.shippingStatus))].sort(), [projections])
  const visible = projections.filter((item) => (selectedMarket === 'ALL' || item.market === selectedMarket) && (shippingStatus === 'ALL' || item.shippingStatus === shippingStatus) && (destination === 'ALL' || item.destination === destination) && (carrier === 'ALL' || item.carrier === carrier) && (!tripSearch.trim() || `${item.trip} ${item.destination ?? ''} ${item.carrier ?? ''}`.toLowerCase().includes(tripSearch.trim().toLowerCase())))
  const upload = async () => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        await onDataChanged?.()
        if (importedWeek) {
          setWeekId(importedWeek)
          await onWeekSelected?.(importedWeek)
          await refreshMarkets(importedWeek)
        }
      }
  ...
  const selectWeek = (value: string) => { setWeekId(value); setSelectedMarket('ALL'); setShippingStatus('ALL'); setDestination('ALL'); setCarrier('ALL'); setExpanded(new Set()); void onWeekSelected?.(value) }
  ...
  return <section className="page projection-page">
    ...
    <section className="panel projection-upload"><div><span className="metric-label">Projection intake</span><h2>Upload a ZIP / TR workbook</h2><p>The filename identifies the market and Projection week: FE Wk 37, BE Wk 37, MMSI Wk 37, or Bost CT Hart Wk 37. ZIP is read from column A and TR from column C. Use .xlsx, .xlsm, .xls, or .csv.</p></div><div className="projection-form"><label>Projection week<select className="week-select" value={weekId} onChange={(event) => selectWeek(event.target.value)} disabled={!availableWeekIds.length}>{availableWeekIds.length ? availableWeekIds.map((id) => <option key={id} value={id}>Week {id}</option>) : <option value="">No uploaded weeks</option>}</select></label><label className="projection-file">ZIP / TR workbook<input ref={input} type="file" accept=".xlsx,.xlsm,.xls,.csv" onChange={() => setMessage('')} /></label><button className="primary-button" type="button" onClick={() => void upload()} disabled={isUploading}>{isUploading ? 'Importing...' : 'Import projection workbook'}</button></div>{message && <p className="projection-message">{message}</p>}</section>
    {!weekId ? <section className="panel empty-page"><h2>No Projection week loaded</h2><p>Upload a Projection workbook to create its week and market tab.</p></section> : <><nav className="production-subnav projection-tabs" aria-label="Projection markets"><button className={selectedMarket === 'ALL' ? 'active' : ''} onClick={() => setSelectedMarket('ALL')}>All markets <span>{projections.length}</span></button>{markets.map((market) => <button className={selectedMarket === market.market ? 'active' : ''} key={market.market} onClick={() => setSelectedMarket(market.market)}>{labels[market.market] ?? market.market} <span>{Object.keys(market.mappings).length}</span></button>)}</nav><div className="shipping-filters projection-filters"><input value={tripSearch} onChange={(event) => setTripSearch(event.target.value)} placeholder="Search trip, destination, or carrier" aria-label="Search trip, destination, or carrier" /><label>Status<select className="status-select" value={shippingStatus} onChange={(event) => setShippingStatus(event.target.value)}><option value="ALL">All</option>{shippingStatuses.map((value) => <option key={value} value={value}>{shippingStatusLabel(value)}</option>)}</select></label><label>Destination<select className="status-select" value={destination} onChange={(event) => setDestination(event.target.value)}><option value="ALL">All</option>{destinations.map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>Carrier<select className="status-select" value={carrier} onChange={(event) => setCarrier(event.target.value)}><option value="ALL">All</option>{carriers.map((value) => <option key={value} value={value}>{value}</option>)}</select></label></div>
    <div className="metric-grid projection-metrics"><Metric label="Trips" value={String(visible.length)} detail={`Projection Week ${weekId}`}/><Metric label="Production ready" value={String(ready)} detail="All ZIPs are processed"/><Metric label="In progress" value={String(partial)} detail="Some ZIPs are processed"/><Metric label="ZIPs in this projection" value={String(visible.reduce((total, item) => total + item.zips.length, 0))} detail="ZIPs included in the displayed trips"/></div>
...
function shippingStatusLabel(status: string) { return status === 'NOT_IN_BULK_PLAN' ? 'Not in Bulk Plan' : status.replaceAll('_', ' ') }
...
function ZipStatus({ status }: { status: ProductionStatus | 'MISSING' }) { return <span className={`status zip-status-${status.toLowerCase()}`}>{status === 'MISSING' ? 'ZIP missing' : status === 'BLOCKED' ? 'SHORT' : status.replaceAll('_', ' ')}</span> }
...
export function ProjectionPage({ weeks, selectedWeekId, token = '' }: { weeks: OperationalWeek[]; selectedWeekId: string; token?: string }) {
  ...
  return <section className="page projection-page">
    <div className="page-heading"><div><h1>Projection</h1><p>Live production readiness for Shipping trips, based on ZIP/TR mappings, job number, and required households.</p></div></div>
    <section className="panel projection-upload"><div><span className="metric-label">ZIP/TR mapping intake</span><h2>Upload a trip-to-production workbook</h2><p>The file name determines its operational week and market. For example, â€œBE Wk 36.xlsxâ€ is imported into Back End for Week 36. ZIP is read from column A and TR from column C.</p></div><div className="projection-form"><label>Shipping week<select className="week-select" value={weekId} onChange={(event) => { setWeekId(event.target.value); setArea('ALL') }} disabled={!weeks.length}>{weeks.length ? weeks.map((item) => <option key={item.id} value={item.id}>{item.label}</option>) : <option value="">No uploaded weeks</option>}</select></label><label className="projection-file">ZIP/TR workbook<input ref={input} type="file" accept=".xlsx" onChange={selectMappingWorkbook} /></label><button className="primary-button" type="button" onClick={() => void upload()} disabled={isUploading || !weekId}>{isUploading ? 'Importingâ€¦' : 'Import ZIP/TR mappings'}</button></div>{message && <p className="projection-message">{message}</p>}</section>
    {!week ? <section className="panel empty-page"><h2>No shipping week selected</h2><p>Upload a Bulk Plan first, then select its shipping week here.</p></section> : <><nav className="production-subnav projection-tabs" aria-label="Projection operations"><button className={area === 'ALL' ? 'active' : ''} onClick={() => setArea('ALL')}>All trips <span>{projections.length}</span></button>{availableAreas.map((item) => <button className={area === item ? 'active' : ''} key={item} onClick={() => setArea(item)}>{areaLabels[item] ?? item} <span>{projections.filter((projection) => projection.areas.includes(item)).length}</span></button>)}</nav>
    <div className="metric-grid projection-metrics"><Metric label="Shipping trips" value={String(visible.length)} detail={`${areaLabels[area] ?? area} for ${week?.label ?? 'selected week'}`}/><Metric label="Production ready" value={String(ready)} detail="All mapped HH complete"/><Metric label="In progress" value={String(inProgress)} detail="Some mapped HH complete"/><Metric label="ZIP/TR coverage" value={`${mapped} / ${visible.length}`} detail={`${sourceTrips.size} mapped trips received; remaining loads await ZIP/TR workbooks`}/></div>
    <section className="panel projection-table"><div className="panel-header"><div><h2>Trip production projection</h2><span>Production changes recalculate these trip bars immediately. A trip without a ZIP/TR mapping is never treated as not started.</span></div><span>{visible.length} visible trips</span></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Trip / load</th><th>Carrier</th><th>Destination</th><th>Required ATZs</th><th>Production progress</th><th>Readiness</th></tr></thead><tbody>{visible.map((projection) => <tr key={projection.load.id}><td>{projection.load.number}</td><td>{projection.load.carrier}</td><td>{projection.load.destination}</td><td>{projection.requirements.length || 'â€”'}</td><td><div className="projection-progress"><div className="progress-label"><span>{projection.requirements.length ? `${projection.completeHH.toLocaleString()} of ${projection.requiredHH.toLocaleString()} HH complete` : 'ZIP/TR mapping required'}</span><strong>{projection.requirements.length ? `${projection.percent}%` : 'â€”'}</strong></div><div className="progress-track"><div className={`progress-fill ${projection.readiness === 'BLOCKED' ? 'projection-blocked' : ''}`} style={{ width: `${projection.percent}%` }} /></div><small>{projection.requirements.length ? `${projection.remainingHH.toLocaleString()} HH remaining Â· ${projection.matched} of ${projection.requirements.length} requirements matched${projection.sources.length ? ` Â· ${projection.sources.join(', ')}` : ''}` : 'Awaiting the ZIP/TR workbook that contains this trip.'}</small></div></td><td><ReadinessBadge readiness={projection.readiness}/></td></tr>)}</tbody></table></div></section></>}</section>
}
...
function deriveProjections(loads: Load[], production: ProductionRecord[], requirements: ProjectionRequirement[]) {
  ...
  return loads.map((load): TripProjection => {
    ...
    const sources = [...new Set(matched.map((record) => `${record.week} Â· ${record.market}`))]
...
function bestProductionMatch(records: ProductionRecord[]) { const priority: Record<ProductionRecord['status'], number> = { COMPLETE: 5, BLOCKED: 4, NOT_STARTED: 3, REWORK: 2, SKIPPED: 1 }; return records.reduce<ProductionRecord | undefined>((best, record) => !best || priority[record.status] > priority[best.status] ? record : best, undefined) }
