  ...
  return <section className="page">
    ...
    <section className="production-sticky">
      <div className="production-summary">
        ...
        <label className="production-search">Search ZIP / ATZ<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="e.g. 02138 C1" /></label>
    ...
    <section className="panel machine-progress"><button className="machine-progress-toggle" type="button" onClick={() => setIsMachineProgressOpen((open) => !open)}><span><strong>Machine progress</strong><small>{machines.length} machines in this area</small></span><span className="toggle-label">{isMachineProgressOpen ? 'Hide details' : 'Show details'}<span aria-hidden="true">{isMachineProgressOpen ? '-' : '+'}</span></span></button>{isMachineProgressOpen && <div className="machine-grid">{machines.map((name) => <MachineCard key={name} name={name} records={areaRecords.filter((record) => record.machine === name)} />)}</div>}</section>
    {markets.map((market) => <MarketTable key={market} market={market} records={visible.filter((record) => record.market === market)} machines={availableMachines} savingId={savingId} movingId={movingId} onStatusChange={changeStatus} onMachineChange={changeMachine} onNotesChange={onNotesChange} />)}
