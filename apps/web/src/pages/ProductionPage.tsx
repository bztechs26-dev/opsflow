...
const statuses: ProductionStatus[] = ['NOT_STARTED', 'COMPLETE', 'BLOCKED', 'SKIPPED']
...
interface Props { records: ProductionRecord[]; queuePlan?: QueuePlan; onStatusChange: (id: string, status: ProductionStatus) => void; onNotesChange: (id: string, notes: string) => void; onQueuePlanChange: (plan: QueuePlan) => void }
...
export function ProductionPage({ records, onStatusChange, onNotesChange }: Props) {
  ...
  return <section className="page">
    ...
    {markets.map((market) => <section key={market} className="market-section"><div className="market-header"><h2>{market}</h2><span>{visible.filter((record) => record.market === market).length} ZIP records</span></div><div className="panel table-wrap"><table className="data-table"><thead><tr><th>ZIP / ATZ</th><th>Machine</th><th>Quantity</th><th>Source status</th><th>Dashboard status</th><th>Notes</th><th>Update status</th></tr></thead><tbody>{visible.filter((record) => record.market === market).map((record) => <tr key={record.id}><td>{record.zip}</td><td>{record.machine}</td><td>{record.volume.toLocaleString()} pcs</td><td>{record.sourceStatus}</td><td><StatusBadge status={record.status}/></td><td><input className="notes-input" maxLength={100} placeholder="Add note" value={record.notes ?? ''} onChange={(event) => onNotesChange(record.id, event.target.value)} /></td><td><select className="status-select" value={record.status} onChange={(event) => onStatusChange(record.id, event.target.value as ProductionStatus)}>{statuses.map((status) => <option key={status} value={status}>{formatStatus(status)}</option>)}</select></td></tr>)}</tbody></table></div></section>)}
...
function recordArea(record: ProductionRecord) { if (record.id.includes('-PROV-BOST-')) return 'PROV-BOST'; if (record.id.includes('-BE-')) return 'BE'; if (record.id.includes('-MMSI-')) return 'MMSI'; return 'FE' }
