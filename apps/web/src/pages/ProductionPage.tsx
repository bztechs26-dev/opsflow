...
function MachineCard({ name, records }: { name: string; records: ProductionRecord[] }) { const capacity = capacityForMachine(name, records); if (!capacity) return null; const markets = [...new Set(records.filter((record) => !['COMPLETE', 'BLOCKED'].includes(record.status)).map((record) => record.market))]; return <div className="machine-item"><div className="progress-label"><span>{name}</span><strong>{capacity.percentComplete}%</strong></div><div className="progress-track"><div className="progress-fill" style={{ width: `${capacity.percentComplete}%` }} /></div><div className="metric-detail"><strong>{capacity.runnablePieces.toLocaleString()} pcs runnable</strong>{markets.length ? ` Â· ${markets.join(', ')}` : ' Â· Complete'}</div><div className="machine-runtime"><strong>{formatRunHours(capacity.estimatedHours)}</strong></div>{(capacity.blockedPieces > 0 || capacity.skippedPieces > 0) && <div className="machine-exceptions">{capacity.blockedPieces > 0 ? `${capacity.blockedPieces.toLocaleString()} pcs short` : ''}{capacity.blockedPieces > 0 && capacity.skippedPieces > 0 ? ' Â· ' : ''}{capacity.skippedPieces > 0 ? `${capacity.skippedPieces.toLocaleString()} pcs skipped` : ''}</div>}<div className="metric-detail">{capacity.completePieces.toLocaleString()} of {capacity.totalPieces.toLocaleString()} pcs processed</div></div> }
...
export function formatStatus(status: string) { return status === 'BLOCKED' ? 'SHORT' : status === 'SKIPPED' ? 'SKIP' : status.replaceAll('_', ' ') }
