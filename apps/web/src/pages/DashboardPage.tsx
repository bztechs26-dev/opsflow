...
import { capacityForMachine, configuredMachineRate, formatRunHours, machineMarketQueue } from '../data/machineCapacity'
...
function MachineCard({ machine, items, activeRate }: { machine: string; items: ProductionRecord[]; activeRate?: number }) { const capacity = capacityForMachine(machine, items, activeRate); if (!capacity) return null; const queue = machineMarketQueue(items); const marketText = queue.map((item) => `${item.market} ${item.pieces.toLocaleString()} pcs`).join(' · '); return <div className="machine-item"><div className="progress-label"><span>{machine}</span><strong>{capacity.percentComplete}%</strong></div><div className="progress-track"><div className="progress-fill" style={{ width: `${capacity.percentComplete}%` }} /></div><div className="machine-queue-line"><strong>{capacity.runnablePieces.toLocaleString()} pcs remaining</strong><span>{queue.length ? `Next: ${queue[0].market}` : 'Queue complete'}</span></div>{queue.length > 0 && <div className="machine-market-queue" title={marketText}>{marketText}</div>}<div className="machine-runtime"><strong>{formatRunHours(capacity.estimatedHours)}</strong></div>{(capacity.blockedPieces > 0 || capacity.skippedPieces > 0) && <div className="machine-exceptions">{capacity.blockedPieces > 0 ? `${capacity.blockedPieces.toLocaleString()} pcs short` : ''}{capacity.blockedPieces > 0 && capacity.skippedPieces > 0 ? ' · ' : ''}{capacity.skippedPieces > 0 ? `${capacity.skippedPieces.toLocaleString()} pcs skipped` : ''}</div>}<div className="metric-detail">{capacity.completePieces.toLocaleString()} of {capacity.totalPieces.toLocaleString()} pcs complete</div></div> }
