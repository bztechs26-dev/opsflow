...
from status_update_api import move_production_zip, update_production_status, update_shipping_hub_assignment, update_shipping_status
...
def handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    if event.get("httpMethod") == "PATCH" and event.get("resource") == "/production/{year}/{week}/{area}/{recordId}/move":
        return move_production_zip(event)
    ...
    if event.get("httpMethod") == "PATCH" and event.get("resource") == "/shipping/{year}/{week}/{loadNumber}/hub-assignment":
        return update_shipping_hub_assignment(event)
